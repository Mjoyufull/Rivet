use crate::login::local_server::spawn_local_server;
...
pub enum LoginFlowStep {
    ...
    #[default]
    Welcome,
    /// SSO login in progress (browser opened).
    SsoInProgress,
    /// Password login form.
    PasswordForm,
}
    ...
    fn handle_sso_login(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this: WeakEntity<Self>, cx| {
            let result = async {
                ...
                match client.complete_sso_login(&callback_url.to_string()).await {
                    Ok(_) => Ok(client),
                    Err(e) => Err(e),
                }
            }
                        ...
                        div().w_full().child(
                            Button::new("password-login")
                                ...
                                .on_click(move |_, _, cx| {
                                    view2.update(cx, |this, cx| this.show_password_form(cx));
                                }),
                        ),
                    ),
            )
                            ...
                            .child(
                                div()
                                    ...
                                    .on_mouse_down(MouseButton::Left, move |_, _, cx| {
                                        view2.update(cx, |this, cx| this.go_back(cx));
                                    }),
                            )
                            .child(
                                div()
